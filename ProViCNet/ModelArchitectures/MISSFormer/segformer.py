...
class MixFFN_skip(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        ax = self.act(
            self.norm1( self.dwconv(self.fc1(x), H, W) + self.fc1(x) )
        )
        out = self.fc2(ax)
        return out
...
class OverlapPatchEmbeddings(nn.Module):
    def __init__(self, img_size=256, patch_size=7, stride=4, padding=1, in_ch=3, dim=768):
        super().__init__()
...
class SegFormer(nn.Module):
    def __init__(self, model_name: str = 'B0', num_classes: int = 19, image_size: int = 256) -> None:
        super().__init__()
