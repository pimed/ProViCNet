...
class WeightedDiceBCE(nn.Module):
    def __init__(self, dice_weight=1, BCE_weight=1, class_weight=None):
    ...
    def forward(self, inputs, targets):
        ...
        for i in range(len(self.class_weight)):
            dice += self.dice_loss(inputs[:, i, :, :], targets[:, i, :, :]) * self.class_weight[i]
            BCE += self.BCE_loss(inputs[:, i, :, :], targets[:, i, :, :]) * self.class_weight[i]
