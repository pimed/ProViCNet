...
def initialize_weights(module):
    for m in module.modules():
        if isinstance(m, nn.Linear):
            ...
            if m.bias is not None:  # 편향이 None이 아닌 경우에만 초기화를 진행
                m.bias.data.zero_()
...
def initialize_MODEL(MODEL, args):
    if args.ModelName in ('ProViDNet', 'ProViNet'):
...
class WeightedBCE(nn.Module):
    ...
    def forward(self, logit_pixel, truth_pixel, target=None):
        # print("====",logit_pixel.size())
        logit = logit_pixel.reshape(-1) #logit = logit_pixel.view(-1)
        truth = truth_pixel.reshape(-1) #truth = truth_pixel.view(-1)
        
        # I made this for evaluating cancer segmentation only in prostate, recommend using this only inference
        if target is not None:
            target = target.reshape(-1)
            logit, truth = logit[target == 1], truth[target == 1]
...
class WeightedDiceLoss(nn.Module):
    ...
    def forward(self, logit, truth, target=None, smooth=1e-5):
        batch_size = len(logit)
        logit = logit.reshape(batch_size,-1) #logit = logit.view(batch_size,-1)
        truth = truth.reshape(batch_size,-1) #truth = truth.view(batch_size,-1)

        # I made this for evaluating cancer segmentation only in prostate, recommend using this only inference
        if target is not None:
            target = target.reshape(batch_size, -1)
            #print("target, logit, truth:", target.shape, logit.shape, truth.shape)
            #logit, truth = logit[target == 1], truth[target == 1]
            logit[ target == 0 ] = 0
            truth[ target == 0 ] = 0
...
class WeightedDiceBCE(nn.Module): # input should be [batch_size, 3, width, height]
    def __init__(self, dice_weight=1, BCE_weight=1, class_weight=None, onlyTarget=True):
    ...
    def forward(self, inputs, truth, target=None, inference_mode=False, verbose=False):
        if inference_mode == False: # Aggregated Loss
            dice, BCE = 0, 0
            for i in range(len(self.class_weight)):
                dice += self.dice_loss(inputs[:, i, :, :], truth[:, i, :, :]) * self.class_weight[i]
                BCE += self.BCE_loss(inputs[:, i, :, :], truth[:, i, :, :]) * self.class_weight[i]
            dice_BCE_loss = self.dice_weight * dice + self.BCE_weight * BCE
            return dice_BCE_loss
        elif inference_mode == True: # [Back, Prostate, CancerDice, CancerBCE]
            Loss_class = []
            for i in range(len(self.class_weight)):
                #if self.onlyTarget and (i == 2): # If it is loss for cancer and inference mode, calculate loss in only prostate
                if i == 2: # If it is loss for cancer and inference mode, calculate loss in only prostate
                    target = truth[:, 1, :, :]
                    #dice = self.dice_loss(inputs[:, i, :, :], truth[:, i, :, :], target) * self.class_weight[i]
                    #BCE  = self.BCE_loss(inputs[:, i, :, :], truth[:, i, :, :], target) * self.class_weight[i] # Only in prostate
                    dice = self.dice_loss(inputs[:, i, :, :], truth[:, i, :, :]) * self.class_weight[i]
                    BCE  = self.BCE_loss(inputs[:, i, :, :], truth[:, i, :, :]) * self.class_weight[i] # Only in prostate
                    Loss_class.append(self.dice_weight * dice)
                    Loss_class.append(self.BCE_weight * BCE)
                else:
                    dice = self.dice_loss(inputs[:, i, :, :], truth[:, i, :, :]) * self.class_weight[i]
                    BCE  = self.BCE_loss(inputs[:, i, :, :], truth[:, i, :, :]) * self.class_weight[i]
                    Loss_class.append(self.dice_weight * dice + self.BCE_weight * BCE)
            return Loss_class
